...
function App() {
  ...
  const genPassword = useCallback(() => {
    let pass = ""
    let str = "ABCDEFGHIJKLMNOPQRSTUVWXYZÅÄÖabcdefghijklmnopqrstuvwxyzåäö"
    ...
    <div className="w-full max-w-md mx-auto shadow-md rounded-lg px-4 py-3 my-8 bg-gray-800 text-orange-500">
      ...
      <div className='flex text-sm gap-x-2'>
        ...
        <div className='flex items-center gap-x-1'>
          <input type="range" min={6} max={32} value={len} className='cursor-pointer' 
          onChange={(e) => setLen(e.target.value)} name='' id=''/>
          <label htmlFor="lenght">Lenght: {len}</label>
        </div>
